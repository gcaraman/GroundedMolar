...
The v1 analyzer now validates and returns selected spawns and persistent actor records as one atomic profile operation. Persistent actor GUID occurrences are located in one complete decoded-save pass, bucketed by the first serialized GUID byte, instead of one complete search per selected actor. The same reference-count and persistent-signature rules remain fail-closed. Deterministic synthetic regressions cover recognized state `1`, an absent persistent record, an unrecognized second record, more than two GUID occurrences, and exactly one actor lookup pass per analysis.

The desktop process lazily initializes and reuses its decoder/analyzer service, so the pinned `ooz.exe` integrity initialization is not repeated on refresh or retry. Parse results and decoded save bytes are not retained by that service. The validated 512 × 512 base-map bitmap and marker icon are also lazily decoded and frozen once per process; confidence and marker-state guards still run before every base-map access.
